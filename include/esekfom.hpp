...
namespace esekfom
{
	...
	public:
						   ...
						   KD_TREE<PointType> &ikdtree, vector<PointVector> &Nearest_Points, bool extrinsic_est)
		{
			...
			laserCloudOri->clear();//存储特征点
			corr_normvect->clear();//存储特征点的法向量

#ifdef MP_EN //开启多线程，近邻点搜索
...
#pragma omp parallel for
			...
			for (int i = 0; i < feats_down_size; i++) //遍历所有的特征点
			{
				...
				vector<float> pointSearchSqDis(NUM_MATCH_POINTS);//存储搜索近邻点的距离
				...
				if (ekfom_data.converge)
				{
					...
					//判断是否是有效匹配点，与loam系列类似，要求特征点最近邻的地图点数量>阈值，距离<阈值  满足条件的才置为true。//点数量大于5，距离不能大于5
				...
				if (esti_plane(pabcd, points_near, 0.1f)) //0.1m
				{
			...
			// 雅可比矩阵H和残差向量的计算
			ekfom_data.h_x = MatrixXd::Zero(effct_feat_num, 12);//mx12
			ekfom_data.h.resize(effct_feat_num);//mx1
												 ...
												 int maximum_iter, bool extrinsic_est)
		{
			...
			dyn_share_datastruct dyn_share;//观测方程残差以及H矩阵
			...
			vectorized_state dx_new = vectorized_state::Zero(); // 24X1的向量，广义加后面的量，就是每次更新量
			...
			for (int i = -1; i < maximum_iter; i++) // maximum_iter是卡尔曼滤波的最大迭代次数
			{
				...
				auto K_front = (HTH / R + P_.inverse()).inverse();//R是常数
				...
				KH.block<24, 12>(0, 0) = K * H;//迭代更新用到KH
				Matrix<double, 24, 1> dx_ = K * dyn_share.h + (KH - Matrix<double, 24, 24>::Identity()) * dx_new; //公式(18)//忽略J
