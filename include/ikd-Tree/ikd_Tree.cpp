...
void KD_TREE::InitTreeNode(KD_TREE_NODE * root){
    ...
    root->alpha_bal = 0.5;
    root->alpha_del = 0.0;
    pthread_mutex_init(&(root->push_down_mutex_lock),NULL); //  用于多线程操作的互斥锁
}   
// 返回kd-tree的大小
int KD_TREE::size(){
    int s = 0;
...
void KD_TREE::Build(PointVector point_cloud){
    ...
    // 创建内存依赖上的根节点STATIC_ROOT_NODE，并初始化为叶节点 
    ...
    // 将逻辑上的根节点指向STATIC_ROOT_NODE的左子节点
...
void KD_TREE::Nearest_Search(PointType point, int k_nearest, PointVector& Nearest_Points, 
    ...
    q.clear();// 清空堆 
    // 临时需要一个对象来调用它的方法，但又不打算在之后使用这个对象
...
// 构建kd-tree，参数为根节点指针、数据点集合的左右边界和点集合对象；
// 这里的二级指针形参很重要，能够允许我们传入一个空指针。
