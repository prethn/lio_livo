...
class LidarSelector {
  public:
    ...
    void dpi(V3D p, MD(2,3)& J); // 计算雅可比矩阵
    float UpdateState(cv::Mat img, float total_residual, int level);
    double NCC(float* ref_patch, float* cur_patch, int patch_size); //计算归一化互相关系数

    void ComputeJ(cv::Mat img); // 计算雅可比矩阵
    void reset_grid();
    ...
    void getWarpMatrixAffine(  // 计算仿射变换矩阵
    ...
    PointCloudXYZI::Ptr pg_down;  // 降采样点云
  ...
  private:
    ...
    typedef std::list<Candidate > Cell;  // 定义候选匹配的列表类型
    typedef std::vector<Cell*> CandidateGrid; // 定义候选网格的向量类型。
...
// 可以方便地创建和共享 LidarSelector 对象，并且在对象不再使用时自动释放其资源。
