...
void standard_pcl_cbk(const sensor_msgs::PointCloud2::ConstPtr &msg)
{
    ...
    p_pre->process(msg, ptr);//通过process()将msg转换成点云
    lidar_buffer.push_back(ptr);//将点云传给lidar_buffer
...
bool sync_packages(MeasureGroup &meas)
{
    ...
    meas.imu.clear();
    while ((!imu_buffer.empty()) && (imu_time < lidar_end_time))//时间是怎么++的？
    {
...
void lasermap_fov_segment()
{
    ...
    //初始化局部地图范围，以pos_LiD为中心,长宽高均为cube_len//局部地图范围
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        // 与某个方向上的边界距离（1.5*450m）太小，标记需要移除need_move(FAST-LIO2论文Fig.3)
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        if (dist_to_map_edge[i][0] <= MOV_THRESHOLD * DET_RANGE)//往左移
        {
            New_LocalMap_Points.vertex_max[i] -= mov_dist;
            New_LocalMap_Points.vertex_min[i] -= mov_dist;
            tmp_boxpoints.vertex_min[i] = LocalMap_Points.vertex_max[i] - mov_dist;//标记删除部分
            cub_needrm.push_back(tmp_boxpoints);
        }
        else if (dist_to_map_edge[i][1] <= MOV_THRESHOLD * DET_RANGE)//往右移
        {
            New_LocalMap_Points.vertex_max[i] += mov_dist;
            New_LocalMap_Points.vertex_min[i] += mov_dist;
            tmp_boxpoints.vertex_max[i] = LocalMap_Points.vertex_min[i] + mov_dist;//标记删除部分
            cub_needrm.push_back(tmp_boxpoints);
        }
    }
    ...
    ikdtree.acquire_removed_points(points_history);//实际上没有用到
...
void set_posestamp(T &out)
{
    ...
    auto q_ = Eigen::Quaterniond(state_point.rot.matrix());//拿取当前状态量的四元数
...
void publish_odometry(const ros::Publisher &pubOdomAftMapped)
{
    ...
    set_posestamp(odomAftMapped.pose);//模板函数
    ...
    br.sendTransform(tf::StampedTransform(transform, odomAftMapped.header.stamp, "camera_init", "body"));//发布tf,完成里程计的发布
}
...
int main(int argc, char **argv)
{
    ...
    nh.param<double>("filter_size_map", filter_size_map_min, 0.5);//体素大小
    ...
    shared_ptr<ImuProcess> p_imu1(new ImuProcess());  //共享指针
    Lidar_T_wrt_IMU << VEC_FROM_ARRAY(extrinT);//平移和旋转的外参
    ...
    while (ros::ok())
    {
        ...
        if (sync_packages(Measures)) //把一次的IMU和LIDAR数据打包到Measures
        {
            ...
            state_point = kf.get_x();//当前状态
            pos_lid = state_point.pos + state_point.rot.matrix() * state_point.offset_T_L_I;//雷达原点在世界坐标系下的位置，更新地图边界
            ...
            //点云下采样
            downSizeFilterSurf.setInputCloud(feats_undistort);//补偿后的点云进行降采样
            downSizeFilterSurf.filter(*feats_down_body);//输出降采样后的点
            feats_down_size = feats_down_body->points.size();//拿到点云大小
            ...
            //初始化ikdtree(ikdtree为空时)，开头几帧是空的
            if (ikdtree.Root_Node == nullptr)//添加点
            {
            ...
            if (0) // If you need to see map point, change to "if(1)"//是否看全局地图
            {
